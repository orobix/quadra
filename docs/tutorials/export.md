...
!!! note
    In the segmentation task, the checkpoint used for export is controlled by `task.checkpoint_selection`. The default value `best` exports the checkpoint with the best monitored metric; setting it to `last` exports the checkpoint from the final training epoch instead. See the [segmentation tutorial](examples/segmentation.md#experiment) for details.
